...
class PeakArea:
    def __init__(
        ...
        self.data = data
        self.peaks_index, self.peaks_dataframe = self.find_fragment_peaks(start=start, end=end, num_peaks=num_peaks)
        self.peak_widths = self.find_fragment_peak_width(rel_height=rel_height)
        self.divided_peaks = self.divide_peaks(padding=padding)
        self.fit_df, self.fit_params, self.fit_report = self.fit_lmfit_model(model_=model)
        self.quotient = self.calculate_quotient()

    def find_fragment_peaks(
        self, start, end, num_peaks: int, threshold: int = 10000
    ):
    ...
    def find_fragment_peak_width(self, rel_height: float = 0.95):
        ...
        return (
            df
            .assign(peak_start=lambda x: np.floor(x.peak_start).astype(int))
    ...
    def plot_lmfit_model(self):
        ...
        for i, ax in enumerate(axs):
            ...
            ax.set_title(f"Area: {self.fit_params[i]['amplitude']: .1f}")
